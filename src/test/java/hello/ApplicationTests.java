...
public class ApplicationTests {
    ...
    //     mockMvc.perform(get("/users").header("Authorization", "Bearer " + token).accept(CONTENT_TYPE))
    ...
    //     mockMvc.perform(get("/users/2").header("Authorization", "Bearer " + accessToken).accept(CONTENT_TYPE))
    ...
    //     mockMvc.perform(get("/users/1").header("Authorization", "Bearer " + accessToken).accept(CONTENT_TYPE))
    //             .andExpect(jsonPath("$.username", is("test")));
    // }

    // private void createUser(JSONObject user, String accessToken) throws Exception {
    //     mockMvc.perform(post("/users").header("Authorization", "Bearer " + accessToken).contentType(CONTENT_TYPE)
    //             .content(user.toString()).accept(CONTENT_TYPE)).andExpect(status().isCreated());
    // }
